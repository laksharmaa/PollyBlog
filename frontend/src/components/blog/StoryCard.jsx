...
export default function StoryCard({ blog, featured = false }) {
	const excerpt = stripHtml(blog.blogContent);
	return (
		<motion.a whileHover={{ y: -4 }} transition={{ duration: 0.2 }} className={featured ? "story-card featured" : "story-card"} href={`/public-blog/${blog.blogId}`}>
