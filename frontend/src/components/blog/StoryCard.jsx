...
export default function StoryCard({ blog, featured = false, href }) {
  ...
  return (
    <motion.a
      ...
      href={href || `/public-blog/${blog.blogId}`}
    >
      {blog.imageUrl && (
      ...
      <div className="card-meta">
        <span>{featured ? "Featured" : "Story"}</span>
        <span>{readingTime(excerpt)} min read</span>
      </div>
      <h3>{blog.blogTitle || "Untitled story"}</h3>
      <p>
        {excerpt.slice(0, featured ? 250 : 170)}
        {excerpt.length > (featured ? 250 : 170) ? "…" : ""}
      </p>
